					...
					<div class="row">
						...
						<form id="form" method="post" action="">
							<table class="table table-striped table-bordered table-hover">
								...
								<tbody>
									<?php if(is_array($data) || $data instanceof \think\Collection || $data instanceof \think\Paginator): if( count($data)==0 ) : echo "" ;else: foreach($data as $key=>$rules): ?>
									<tr>
										<td class="center">
											<?php echo $rules['id']; ?>
										</td>
										<td><?php echo $rules['per_name']; ?></td>
										<td><?php echo $rules['address']; ?></td>
										<td><i class="menu-icon fa fa-<?php echo $rules['icon']; ?>"></i></td>
										<td class="center">
											<?php if($rules['is_show'] == 1): ?>
										...
										<td class="center">
											<?php if($rules['status'] == 1): ?>
											<span class="label label-sm label-success">启用</span>
											<?php else: ?>
											<span class="label label-sm label-danger">禁用</span>
										<?php endif; ?>
										</td>
										<td><?php echo $rules['sort']; ?></td>
										<td class="center">
											<a href="<?php echo url('rule/edit',array('id'=>$rules['id'])); ?>">修改</a>&nbsp;|
											<a class="del" href="javascript:;" title="删除" data-id="<?php echo $rules['id']; ?>">删除</a>
										</td>
									</tr>							
									<?php endforeach; endif; else: echo "" ;endif; ?>
								</tbody>
