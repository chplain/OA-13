		...
		<div class="page-content">
			<div class="row">
				<div class="col-xs-12">
					<div class="row">
						<div class="cf">
							<a class="btn btn-info" href="<?php echo url('week/add'); ?>">新增</a>
						</div>
							...
							<table class="table table-striped table-bordered table-hover">
								...
								<tbody>
									...
									<tr>
										...
										<td  class="center"><?php echo $name[$value['add_userid']]; ?></td>
										<td  class="center"><?php echo $type[$value['type_id']]; ?></td>
										<td  class="center"><?php echo date("Y-m-d H:s",$value['add_time'] ); ?></td>
										<td  class="center status"><?php if($value['status'] == 0): ?>新提交<?php elseif($value['status'] == 1): ?>已审批<?php elseif($value['status'] == 2): ?>驳回<?php endif; ?></td>
										<td  class="center">
											<?php if($value['add_userid'] != \think\Session::get('userinfo.id')): ?>
											<a href="<?php echo url('week/cat',array('id'=>$value['id'])); ?>">查看</a>
											<?php else: ?>
											<a href="<?php echo url('week/edit',array('id'=>$value['id'])); ?>" <?php if($value['status'] == 1): ?>style="pointer-events: none; color:#999;"<?php endif; ?>>修改</a>&nbsp;|
...
<script type="text/javascript">
	$(".del").click(function(){
		...
		layer.confirm("确定要删除该用户吗?",function(index){
			// alert(index);
			$.get('<?php echo url("week/del"); ?>',{wid:id},function(res){
